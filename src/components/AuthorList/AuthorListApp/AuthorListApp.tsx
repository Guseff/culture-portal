...
class AuthorListApp extends Component<AuthorListAppProps, AuthorListAppState> {
    ...
    handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (e.target.value !== '') {        
          ...
          newList = currentList.filter((item: any) => {               
            const filter = filterSearch === 'Имя' ? item.name.toLowerCase() : item.birthCity.toLowerCase();    
    ...
    render() {
      ...
      return (
        <Container>
          <AuthorListSearch handleFilterSelect={this.handleFilterSelect} handleInputChange={this.handleInputChange}/>
